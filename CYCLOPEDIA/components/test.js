...
const [previewUri, setPreviewUri] = useState(null);
  ...
  const proceed = () => {
    if (photos.length !=8) {
      Alert.alert('No Photos', 'Please add at least one photo to proceed.Invalid Photo Count', 'Please add exactly 8 photos to proceed.');
      return;
      navigation.navigate('NextScreen', { photos });
    }

    // Navigate to the next screen with the photos
    navigation.navigate('NextScreen', { photos });
  };
